            ...
            users = self.vkapi.method('users.search',
                                      {
                                          ...
                                          'has_photo': True,
                                          'age_from': params['year'] - 3 if params['year'] is not None else None,
                                          'age_to': params['year'] + 3 if params['year'] is not None else None,
                                      }
                                      )
        except ApiError as e:
            users = []
...
if __name__ == '__main__':
    ...
    #  datetime.now().year - int(info.get('bdate').split('.')[2])
    #                if info.get('bdate') is not None else None